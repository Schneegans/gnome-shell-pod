...
# Capture the xvfb screen.
convert xwd:/opt/Xvfb_screen0 ${WORK_DIR}/screen.png

# Reduce the size of the screen capture and the target to optimize the search performance.
convert ${WORK_DIR}/screen.png -resize $((100 / SCALE_FACTOR))% "${WORK_DIR}/screen_small.png"
...
# Perform the crop.
convert "${WORK_DIR}/screen.png" -crop ${CROP_WIDTH}x${CROP_HEIGHT}+${CROP_X}+${CROP_Y} \
